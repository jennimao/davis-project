// NOTE: there is a bug that occurs the first time we press down
// it swings up first quickly before going down 
// need to potentially correct for that in this version 
...
void loop() {
  ...
  if (mySerial.available() ) // if recieve data
  {
     ...
     if (data == 4 && not up)
     {
      ...
      long angle = map(progress, 0, MOVING_TIME, 0, 75);
      servo1.write(90 + angle); 
      servo2.write(100 - angle); 
    }
    up = true;
    progress = 0;
     }
     ...
     if (data == 5 && up)
     {
        ...
        while (progress <= MOVING_TIME) {
          progress = millis() - moveStartTime;
          long angle = map(progress, 0, MOVING_TIME, 75, 0);
          servo1.write(90 + angle); 
          servo2.write(100 - angle); 
        }
        progress = 0;
